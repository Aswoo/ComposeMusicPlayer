...
class BasicWidgetProvider : AppWidgetProvider() {
    ...
    private fun setupPendingIntents(context: Context, remoteViews: RemoteViews) {
        ...
        // 이전 곡 버튼
        val previousIntent = Intent(context, MediaSessionBroadcastReceiver::class.java).apply {
        ...
        // 재생/일시정지 버튼
        val playPauseIntent = Intent(context, MediaSessionBroadcastReceiver::class.java).apply {
        ...
        // 다음 곡 버튼
        val nextIntent = Intent(context, MediaSessionBroadcastReceiver::class.java).apply {
            action = "ACTION_NEXT"
        }
    ...
    private fun updateMetadata(context: Context, remoteViews: RemoteViews, metadata: MediaMetadata, isPlaying: Boolean) {
