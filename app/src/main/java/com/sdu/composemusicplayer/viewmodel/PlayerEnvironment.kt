...
class PlayerEnvironment @Inject constructor(
    ...
    override fun observeQueue(): Flow<List<Music>> = _musicQueue.map { queue ->
...
//        queueRepository.saveQueueFromDBQueueItems(newItems)
    ...
    override suspend fun refreshMusicList() {
        val freshList = musicRepository.getAllMusics()
            .first()
    ...
    override suspend fun play(music: Music, playSource: PlaySource, playList: List<Music>?) {
        when (playSource) {
            PlaySource.PLAYLIST -> {
                val queue = _musicQueue.value
